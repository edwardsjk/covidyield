...
#' @param data Input dataset with one record per individual not previously diagnosed with COVID-19
#' @param y name of variable with indicator that COVID-19 test is positive
...
covidyieldstrategy <- function(data, y, strategyind, strata = NULL, weight = "No Weights", cluster = "No Clusters"){
  require(survey)
  dat <- data
  # dat$y <- y
  # dat$str <- strategyind
...
#' Estimate COVID-19 testing yield under a number of strategies
#'
#' This function summarizes results across multiple strategies
#' @param data Input dataset
#' @param y indicator that COVID-19 test is positive
#' @param strategyind name of variable indicating whether or not a specific person would be tested under a given strategy
...
covidcalcyield<- function(data, y, strategies,  weight = "No Weights", cluster = "No Clusters", stratlabels = NULL){
  ...
  for(i in 1:num){
    str <- strategies[i]
    #stri <- data[,str]
    res[i,] <- covidyieldstrategy(data = data, y = y, strategyind = str, weight = weight, cluster = cluster)
  }
  ...
  res2$Yield <- formatC(round(res2$Yield), digits = 0, format = "f")
  res2$NNT <- formatC(round(res2$NNT), digits = 0, format = "f")
  res2 <- res2[, c(7, 1, 2, 3, 4, 5, 6)]
  return(res2)
}

#' A function to plot expeceted COVID-19 testing yield by strategy
#'
#' This function plots the number needed to test to identify 1 new (previously undiagnosed) case of COVID-19 and 95% CI for each strategy
#' @param results Matrix of results from covidcalcyield function
#' @export
#' @examples
#' covidplotyield(allresults)
